...
export default function ArmazemPage() {
    ...
    const [aba, setAba] = useState<'estoque' | 'parados' | 'incidentes'>('estoque')
    ...
    async function carregarDados(cid: string) {
        ...
        const [pkgsRes, incRes] = await Promise.all([
            supabase.from('packages')
                .select('id, barcode, status, created_at, clients(name)')
                .eq('company_id', cid)
                .in('status', ['in_warehouse', 'unsuccessful'])
        ...
        // Estoque normal — in_warehouse
        setEstoque(pkgs.filter((p: any) => p.status === 'in_warehouse'))

        // Parados — in_warehouse há mais de 3 dias
        setParados(pkgs.filter((p: any) => p.status === 'in_warehouse' && p.diasParado >= 3))

        // Parados com motorista — unsuccessful aguardando retorno
                ...
                <div className="flex gap-2 mb-6 flex-wrap">
                    {[
                        ...
                        <button key={a.key} onClick={() => setAba(a.key as any)}
                            ...
                            style={{
                                backgroundColor: aba === a.key ? '#00b4b4' : '#1a2736',
                                color: 'white'
                            }}>
                            {a.label}
                        </button>
                            ...
                            <div className="flex flex-col gap-4">
                                ...
                                <div className="grid grid-cols-2 gap-3">
                                    {Object.values(estoquePorCliente).map(c => (
                                        <div key={c.nome} className="rounded-lg p-4"
                                            ...
                                            <div className="flex gap-3 mt-2 text-xs font-bold">
                                                {c.criticos > 0 && (
                                                    <span style={{ color: '#ff5252' }}>🔴 {c.criticos} críticos</span>
                                                )}
                                                {c.alertas > 0 && (
                                                    <span style={{ color: '#ffb300' }}>🟡 {c.alertas} alerta</span>
                                                )}
                                                {c.criticos === 0 && c.alertas === 0 && (
                                    ...
                                    {Object.keys(estoquePorCliente).length === 0 && (
                                        <div className="col-span-2 rounded-lg p-8 text-center"
                                            style={{ backgroundColor: '#1a2736' }}>
                                            ...
                                            {estoque.map(p => (
                                                <div key={p.id} className="flex items-center justify-between p-3 rounded"
                                                    ...
                                                    <div className="flex items-center gap-2">
                                                        ...
                                                        {p.diasParado >= 3 && (
                                                            <button
                                                                onClick={() => { setPacoteSelecionado(p); setModalIncidente(true) }}
                        ...
                        {aba === 'incidentes' && (
                            <div className="flex flex-col gap-3">
                                {incidentes.length === 0 ? (
                                    ...
                                    incidentes.map(inc => (
                                        <div key={inc.id} className="rounded-lg p-4"
                                            style={{ backgroundColor: '#1a2736' }}>
